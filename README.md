# 🛡️ DarkShield — AI-Powered Parametric Income Protection for Q-Commerce Delivery Partners
...
| **Trigger Logic** | Disruption must affect the rider's specific dark store pickup point |
| **Income Baseline** | Built from rider's personal delivery completion history at that specific dark store |
| **Fraud Detection** | Dark store fixed-origin model makes GPS spoofing uniquely detectable |
...
A local protest blocks the 3 roads leading to Arjun's Andheri dark store. Police impose a 2-hour movement restriction. DarkShield's social disruption monitor detects the curfew via news API + traffic anomaly data, validates Arjun's location relative to the store, and initiates a partial income payout for the blocked hours.

**Scenario C — Extreme Heat Shutdown**

During a Delhi heatwave (47°C), Zepto suspends deliveries from 11AM–4PM for rider safety. Vikram loses his entire afternoon shift. DarkShield detects the platform-level suspension via mock API + IMD heat alert and automatically pays his afternoon income baseline.
...
| Dark Store Risk Score | 0.7x – 1.8x | Flood/heat/curfew risk score at that store's GPS coordinates |
...
> ⚠️ **CRITICAL SECURITY ARCHITECTURE** — Added in response to GPS spoofing syndicate threat report
...
Our AI model fuses all five signals into a **Genuine Stranding Score (0–100)**. Claims below 40 are auto-rejected. Claims between 40–70 enter a fast-track manual review queue. Claims above 70 are auto-approved.
...
- Sudden spike in claims from one store within a 10-minute window triggers a syndicate alert
- Velocity check — a rider who has never claimed before suddenly claiming on their first eligible disruption gets enhanced review

### 7.3 The UX Balance: Protecting Honest Riders

Our biggest design challenge is ensuring that legitimate riders in genuine network-degraded conditions (heavy rain kills cell signal) are not unfairly penalised.
...
```
Step 1 — Onboarding
  → Rider opens DarkShield PWA
  → Enters Zepto/Blinkit Partner ID, selects primary dark store from map
  → System pulls 28-day earnings history via mock platform API
  → AI risk engine scores the store, generates personalised weekly premium
  → Rider pays via UPI → covered from next Monday

Step 2 — Weekly Policy
  → Policy auto-renews every Monday with recalculated premium
  → Rider notification: "Your dark store risk this week: Medium. Premium: ₹34. Coverage: up to ₹4,200."
  → 1-tap pause for vacation weeks

Step 3 — Disruption Detection & Auto-Claim
  → System monitors 5 data feeds continuously at each registered dark store's GPS pin
  → Trigger threshold crossed → Genuine Stranding Score calculated
  → Score >70: Auto-claim initiated, rider notified immediately
  → Score 40–70: Fast-track review, rider notified within 2 minutes
  → Score <40: Verification requested from rider

Step 4 — Payout
  → Approved claim → payout calculated (hours affected × daily baseline × severity %)
  → Funds via Razorpay (test mode) / UPI Simulator
  → Rider notification: "DarkShield paid you ₹408 for 3 hours lost to flooding at Koramangala store."

Step 5 — Dashboard
  → Rider view: Weekly earnings protected, active coverage, claim history, store risk level
  → Admin view: Store-wise loss ratios, fraud alert queue, weekly disruption forecast, premium adequacy
```
